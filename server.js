...
async function blingGet(token, path, tentativa = 1) {
  ...
  if (result.status === 429 && tentativa <= 5) {
...
app.get('/api/sync', async (req, res) => {
  ...
  try {
    ...
    // 4. Detalhes dos pedidos em lotes de 3
    const pedidos = [];
    ...
    console.log('[sync] concluido: ' + pedidos.length + ' pedidos');
...
app.get('/debug/pedido/:id', async (req, res) => {
  ...
  res.json(await blingGet(token, '/pedidos/vendas/' + req.params.id));
});
...
app.get('/debug/depositos', async (req, res) => {
  ...
  res.json(await blingGet(token, '/depositos'));
});
...
app.get('/debug/saldos', async (req, res) => {
  ...
  res.json(await blingGet(token, path));
});
...
// Proxy para syncLeve do frontend
app.use('/api/bling', async (req, res) => {
  let token;
  try { token = await ensureToken(req); }
  catch(e) { return res.status(401).json({ error: 'Nao autenticado' }); }
  const qs = req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : '';
  const blingPath = req.path + qs;
  const r = await blingGet(token, blingPath);
  res.json(r);
});
